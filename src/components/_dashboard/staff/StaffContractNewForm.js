...
import { Box, Card, Grid, Stack, Switch, TextField, Typography, FormHelperText, FormControlLabel, Avatar, TextareaAutosize, Autocomplete } from '@mui/material';
// utils
...
export default function StaffContractNewForm({ isEdit, currentContract }) {
  ...
  const NewStaffSchema = Yup.object().shape({
    ...
    Salary: Yup.number().required('Salary is required').min(Number(4000000)),
    ...
    onSubmit: async (values, { setSubmitting, resetForm, setErrors }) => {
      try {
        ...
        if (isEdit) {
          ...
          dispatch(editContract(values.id, data))
        } else {
          data = {
